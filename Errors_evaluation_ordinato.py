...
def perform_convAE(train_FOM: np.ndarray, test_FOM: np.ndarray, dump_path: str, weights: Optional[np.ndarray] = None) -> Tuple[np.ndarray, np.ndarray]:
    """_summary_
    ...
    tensor_test = np.expand_dims(test_FOM, axis=1) 
    tensor_train = np.expand_dims(train_FOM, axis=1)     
...
def perform_POD(train_data: np.ndarray, test_data: np.ndarray, rank: int, weights: Optional[np.ndarray] = None) -> Tuple[np.ndarray, np.ndarray]:
    """_summary_
    ...
    train_POD = train_data.reshape(len(train_data), -1)
    test_POD = test_data.reshape(len(test_data), -1)
...
def perform_POD_NN(train_FOM: np.ndarray, test_FOM: np.ndarray, params_training:np.ndarray, params_testing:np.ndarray, rank:int, ann:ANN, dump_path:str)  -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    """
    train_data = train_FOM.reshape(len(train_FOM), -1)
    test_data = test_FOM.reshape(len(test_FOM), -1)
    if os.path.exists(dump_path):
...
for test in types_test:
    ...
    train_FOM, test_FOM, params_training, params_testing = prepare_data(test, 'Data')
    ...
    ### SETTING CORRECT DIMENSIONS FOR CONV_AE (1 CHANNEL) ###
    """
    CONV_AE
    ...
    #since the number  of elements in the first dimension remains the same, the remaining dim has n_elements/n_rows 
    #(n_elements = total elements in the structure)
    train_POD = train_FOM.reshape(len(train_FOM), -1)
    test_POD = test_FOM.reshape(len(train_FOM), -1)
    """

    ### CREATING THE RESULTS FOLDER ###

    path = f'./Stochastic_results/{test}_tests/{test}_{conv_layers}_conv/conv_AE_{epochs}epochs'
    ...
    ### SET THE ANALYSIS FRAMEWORK ###

    neurons_linear = [14]    # 2, 6, 14, 30
    models = ["convAE-FOM"]#, ["POD","POD-NN","NN-encoder", "convAE","wPOD", "wconvAE"]
    ...
    for dim in neurons_linear:
        ...
        directory = f'/{dim}linear_neurons' 
        path = os.path.join(path, directory)
        
            
        if not (os.path.isfile(f"{path}/model_{test}_{conv_layers}conv_{epochs}ep_{dim}lin_neurons.pt")):
            ...
            torch.save(conv_ae, f'{path}/model_{test}_{conv_layers}conv_{epochs}ep_{dim}lin_neurons.pt')
            ...
            conv_ae = torch.load(f'{path}/model_{test}_{conv_layers}conv_{epochs}ep_{dim}lin_neurons.pt') 
