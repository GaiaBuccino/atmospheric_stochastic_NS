...
class Denoising_loss(nn.Module):
    ...
    def __init__(self, original_snap = None):
        super().__init__()
        self.original_snap = original_snap

    def forward (self,input, target):

        original_snapshots = input.reshape(len(input), -1)
        #input = input.reshape(len(input), -1)
        target = target.reshape(len(target), -1)

        return torch.nn.MSELoss(original_snapshots, target)
...
class convAE(Reduction, ANN):
    ...
    def __init__(self,
        ...
        elif loss is not None and original_snap is not None:
            loss = Denoising_loss(original_snap)
    ...
    def fit(self, values, weights = None, original_snap = None): #aggiungere i pesi
        """
        ...
        if weights is None & original_snap is None:
            self.loss = torch.nn.MSELoss()
        elif weights is not None & original_snap is None:
            self.loss = Weighted_loss(weights)
        elif original_snap is not None & weights is None:
            self.loss = Denoising_loss(original_snap)
