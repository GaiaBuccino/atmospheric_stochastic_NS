...
for n_dense in [6,14,30]: 


    for n_batch  in [4,16,180]:
        string_file = "trial_%03d_batch_%03d_n_dense_%03d"%(trial,n_batch,n_dense)
        ...
        if trial==1:
            class CNNModel(nn.Module):
                def __init__(self):
                    ...
                    ### Linear section
                    self.lin_encoder = nn.Linear(512, n_dense)

                    self.lin_decoder = nn.Linear(n_dense, 512)
        ...
        with open("model_details_%s.txt"%string_file, 'w') as f:
            f.write(str(model))

        with open("errors_param_%s.txt"%string_file, 'w') as f:
            f.write("Param   error \n")
            for batch_idx, (data, labels) in enumerate(test_loader):
                data=data.to(device)
                optimizer.zero_grad()
                ...
                for i in range(len(labels)):
                    error = np.mean(np.abs((recon_batch[i, :, :, :]-data[i, :, :, :]).view(grid_size, grid_size).detach().cpu().numpy()))
                    f.write("%1.15f  %1.16e \n"%(labels[i],error))
