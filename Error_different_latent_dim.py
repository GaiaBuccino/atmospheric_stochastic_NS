...
conv_ae = convAE([fake], [fake], f(), f(), epochs, neurons_linear)  #fake structure
conv_ae = torch.load(f'./Stochastic_results/conv_AE_{epochs}epochs_{conv_layers}_conv_layers.pt')
...
for i in range(len(e_test_snapshots)):
    ...
    model_testing_err[i] = np.linalg.norm(e_test_snapshots[i] - test_data[i])/np.linalg.norm(test_data[i])

for i in range(len(e_train_snapshots)):
   
    model_training_err[i] = np.linalg.norm(e_train_snapshots[i] - train_data[i])/np.linalg.norm(train_data[i])
    

#plot error TESTING
plt.figure()
plt.semilogy(params_testing, model_testing_err, 'ro-')
plt.title(f"Reconstruction of the Testing FOM with convAE_{epochs}epochs")
plt.ylabel("w relative error")
plt.xlabel("time")

plt.savefig(f'./Stochastic_results/Snapshots{conv_layers}_conv/linear_{neurons_linear}_neurons/Reconstruction_Testing_FOM_convAE{epochs}epochs.pdf', format='pdf',bbox_inches='tight',pad_inches = 0)
plt.close()

#plot error TRAINING
plt.figure()
plt.semilogy(params_training, model_training_err, 'bo-')
plt.title(f"Reconstruction of the Training FOM with convAE_{epochs}epochs")
plt.ylabel("w relative error")
plt.xlabel("time")

plt.savefig(f'./Stochastic_results/Snapshots{conv_layers}_conv/linear_{neurons_linear}_neurons/Reconstruction_Training_FOM_convAE{epochs}epochs.pdf', format='pdf',bbox_inches='tight',pad_inches = 0)
plt.close()
